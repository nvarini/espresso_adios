...
SUBROUTINE extrapolate_wfcs( wfc_extr )
  ...
  LOGICAL :: exst
#if defined __ADIOS 
  character(len=25)  :: filename_old = "oldwfc.bp"
  character(len=25)  :: filename_old2 = "oldwfc2.bp"
  character(len=25)  :: filename_old3 = "oldwfc3.bp"
  character(len=25)  :: filename_old4 = "oldwfc4.bp"
!integer*8, dimension(2) :: offset=0, readsize=1
  ...
  !
#if defined __ADIOS 
  !CALL adios_init("/home/nvarini/Dragoni_IO/espresso/PW/src/pwdesc.xml",adios_comm,adios_err)
  !call adios_read_init_method (method, adios_comm, "verbose=3", adios_err);
...
#if defined __ADIOS
        !IF(ik.eq.1)THEN!here we write evc on filename_old
        !  call adios_open (adios_handle, "wfc", filename_old, "w", adios_comm, adios_err);
        !ELSE
        !  call adios_open (adios_handle, "wfc", filename_old, "a", adios_comm, adios_err);
        !ENDIF
        CALL write_adios_evc(ik,adios_handle,"wfc",filename_old,adios_comm)
  ...
  ELSE 
     ...
     DO ik = 1, nks
        ...
        !
#if defined __ADIOS
    !CALL adios_read_open_file(f, filename_old, method, adios_comm, ierr);
    !CALL adios_schedule_read(f, sel, "evc", ik-1, 1, evcold, ierr)
    !CALL adios_perform_reads (f, ierr)
    !CALL adios_read_close (f, ierr)
    CALL adios_read_array(ik,f,filename_old,adios_comm,"evc",evcold)
#else
    CALL davcio( evcold, 2*nwordwfc, iunoldwfc, ik, -1 )
...
#if defined __ADIOS
        !IF(ik.eq.1)THEN
        !  call adios_open (adios_handle, "oldwfc", filename_old3, "w", adios_comm, adios_err);
        !ELSE
        !  call adios_open (adios_handle, "oldwfc", filename_old3, "a", adios_comm, adios_err);
        !ENDIF
        CALL write_adios_evcold(ik,adios_handle,"oldwfc",filename_old3,adios_comm,evcold)
        ...
        IF ( wfc_order > 2 ) THEN
           ...
           IF ( wfc_extr == 3 ) THEN
#if defined __ADIOS
             !CALL adios_read_open_file(f, filename_old2, method, adios_comm, ierr);
             !CALL adios_schedule_read(f, sel, "evcold", ik-1, 1, evcold, ierr)
             !CALL adios_perform_reads (f, ierr)
             !CALL adios_read_close (f, adios_err)
             !CALL mpi_barrier(adios_comm,ierr)
             CALL adios_read_array(ik,f,filename_old2,adios_comm,"evcold",evcold)
...
#if defined __ADIOS
        !IF(ik.eq.1)THEN
        !  call adios_open (adios_handle, "oldwfc2", filename_old4, "w", adios_comm, adios_err);
        !ELSE
        !  call adios_open (adios_handle, "oldwfc2", filename_old4, "a", adios_comm, adios_err);
        !ENDIF
        CALL write_adios_aux(ik,adios_handle,"oldwfc2",filename_old4,adios_comm,aux)
...
#IF DEFINED(__ADIOS)
     DO ik=1, nks
       !CALL adios_read_open_file(f, filename_old3, method, adios_comm, ierr);
       !CALL adios_schedule_read(f, sel, "evcold", ik-1, 1, evc, ierr)
       !CALL adios_perform_reads (f, ierr)
       !CALL adios_read_close (f, ierr)
       CALL adios_read_array(ik,f,filename_old3,adios_comm,"evcold",evc)
       !IF(ik.eq.1)THEN
       !  call adios_open (adios_handle, "wfc", filename_old, "w", adios_comm, adios_err);
       !ELSE
       !  call adios_open (adios_handle, "wfc", filename_old, "a", adios_comm, adios_err);
       ...
       !CALL adios_read_open_file(f, filename_old4, method, adios_comm, ierr);
       !CALL adios_schedule_read(f, sel, "aux", ik-1, 1, evcold, ierr)
       !CALL adios_schedule_read(f, sel, "evcold", ik-1, 1, evcold, ierr)
       !CALL adios_perform_reads (f, ierr)
       !CALL adios_read_close (f, ierr)
       !IF(ik.eq.1)THEN
       !  call adios_open (adios_handle, "oldwfc", filename_old2, "w", adios_comm, adios_err);
       !ELSE
       !  call adios_open (adios_handle, "oldwfc", filename_old2, "a", adios_comm, adios_err);
       !ENDIF
       !include "gwrite_oldwfc.fh"
       !call adios_close (adios_handle, adios_err)
       CALL adios_read_array(ik,f,filename_old4,adios_comm,"aux",evcold)
       CALL write_adios_evcold(ik,adios_handle,"oldwfc",filename_old2,adios_comm,evcold)
     ENDDO
#ENDIF
     !
     IF ( zero_ew > 0 ) &
